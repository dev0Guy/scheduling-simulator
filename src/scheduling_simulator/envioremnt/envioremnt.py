...
class SchedulingEnviorment(gym.Env['ObservationDict', int]):
    ...
    def __init__(
        ...
        max_n_machines: Optional[int] = None,
    ) -> None:
        super().__init__()
    ...
    def _cast(self, observation: 'Observation') -> 'ObservationDict':
        ...
        # Pad machines to max_n_machines
        n_machines_actual = d['machines_usage'].shape[0]
        m_pad = self._max_n_machines - n_machines_actual
        machines_usage = d['machines_usage']
        machines_capacity = d['machines_capacity']
        if m_pad > 0:
            machines_usage = np.concatenate([
                machines_usage,
                np.zeros((m_pad, machines_usage.shape[1], machines_usage.shape[2]), dtype=machines_usage.dtype)
            ])
            machines_capacity = np.concatenate([
                machines_capacity,
                np.zeros((m_pad, machines_capacity.shape[1], machines_capacity.shape[2]), dtype=machines_capacity.dtype)
            ])

        return {
            'machines_usage': machines_usage,
            'machines_capacity': machines_capacity,
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple['ObservationDict', Information]:
        ...
        self._n_machines_actual = self._config['n_machines']
    ...
    def step(self, action: int) -> tuple['ObservationDict', float, bool, bool, Information]:
        ...
        if action == 0:
            ...
            if machine_idx >= self._n_machines_actual or padded_job >= self._n_actual:
                cluster_action = 0
            else:
    ...
    def action_masks(self) -> np.ndarray:
        ...
        n_machines = self._max_n_machines
        n_machines_actual = self._n_machines_actual
        ...
        # Only actual machines have valid candidates
        fits = fits[:n_machines_actual]
        mask = np.zeros((n_machines, self._max_n_jobs), dtype=bool)
        mask[:n_machines_actual, :n_actual] = fits & pending[None, :]
