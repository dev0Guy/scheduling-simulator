...
class CustomMetricsCallback(BaseCallback):
    ...
    def __init__(self, verbose=0):
        ...
        self.episode_metrics = EpisodesMetrics(
                count=0, allocations=[], length=[], reward=[],
                avg_wait_time=[], max_wait_time=[], current_time=[],
                failed_allocations=[], completion_time=[],
            )
        self._current_failed_allocations = 0
    ...
    def _on_step(self) -> bool:
        ...
        for env_idx, (done, info) in enumerate(zip(dones, infos)):
            ...
            if action_success == 0:
                self._current_failed_allocations += 1
            ...
            self.episode_metrics["completion_time"].append(np.average(wait_time + ttl))
            self.episode_metrics["allocations"].append(int(np.sum(status != JobStatus.PENDING)))
            self.episode_metrics["failed_allocations"].append(self._current_failed_allocations)
            self._current_failed_allocations = 0
